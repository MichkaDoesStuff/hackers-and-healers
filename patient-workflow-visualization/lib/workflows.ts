...
interface Template {
  steps: Array<Omit<WorkflowStepData, never> & { x: number; y: number }>
}
...
const TEMPLATES: Record<IssueCategory, Template> = {
  lab: {
    steps: [
      ...
      { kind: "draft", title: "Draft clinician message", detail: "Claude drafts a summary + recommended action", actor: "Loop AI", x: 560, y: 0 },
  ...
  billing: {
    steps: [
      ...
      { kind: "draft", title: "Draft correction", detail: "Add prior-auth modifier to each line", actor: "Loop AI", x: 560, y: 100 },
  ...
  medication: {
    steps: [
      ...
      { kind: "draft", title: "Draft adjustment", detail: "Propose dose change or substitution", actor: "Loop AI", x: 560, y: 100 },
...
export function buildWorkflow(issue: Issue): { nodes: StepNode[]; edges: Edge[] } {
  ...
  const nodes: StepNode[] = tpl.steps.map((s, i) => ({
    ...
    data: { kind: s.kind, title: s.title, detail: s.detail, actor: s.actor },
  }))
